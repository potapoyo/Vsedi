...
通常の配布版では `INFO` 以上を基本とする。ユーザーは設定画面から `ERROR` / `WARN` / `INFO` / `DEBUG` / `TRACE` を変更でき、変更は再起動せずに適用する。`DEBUG` / `TRACE` はユーザーが明示的に有効化した診断モード等でのみ使用する。
...
ログ表示画面は、調査中の情報を欠落させないため、保持期間内のサニタイズ済みログをすべて表示する。
