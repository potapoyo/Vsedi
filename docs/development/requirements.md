...
- 無償で利用可能で、運用上許容できるコード署名手段がある場合は積極的に利用する
- macOS Apple Silicon 版では、正式な Developer ID を利用しない場合でも Tauri / macOS が対応するアドホック署名を利用できる構成にする
- Apple の Notarization（公証）は有料 Apple Developer Program が必要となるため、無償配布段階では必須要件にしない
- Windows の信頼済みコード署名についても、有料証明書の購入を必須要件にはしない。無償で利用可能な信頼できる手段が採用可能かを公開ベータ前に再調査する
